...
namespace GL {
	...
	bool GlManager::CreateX3pModel(unsigned int width, unsigned int height, float xoffset, float yoffset, float* zpointData, float minZ, float maxZ) {
		...
		lightControl->radiusMax = (lightMax / 2.0f) + 3.0f;//(lightMax / 2.0f) + 2
		lightControl->lightPos = glm::vec3(centerPos.x, centerPos.y + lightControl->radiusMax, centerPos.z);
	...
	void GlManager::Render(Data& data) {
		...
		for (int i = 0; i < models.size(); ++i) {
			...
			mposition = data.reset ? model->ReSetPoisition() : model->UpdatePoisition(data, cmaera);
		...
		/*	shader->SetShaderFloat(data.zFactor, "zFactor");*/
			...
			auto rr = data.colors[1][0];
			auto gg = data.colors[1][1];
			auto bb = data.colors[1][2];
			//模型颜色
			...
			//光照
			float ccc = data.colors[2][0];
			float ccc2 = data.colors[2][1];
			float ccc3 = data.colors[2][2];
			...
			if (data.lightType == SPOT_LIGHT) { //聚光
				...
				glm::vec3 lpos = lightControl->lightPos;
				lpos = glm::vec3(mposition * glm::vec4(lpos, 1.0f));
				shader->SetShaderVec3(lpos, "light.position");//光源位置
		...
		if (data.showLightMode) {
			...
			glm::vec3 lpos = lightControl->lightPos;
			lpos = glm::vec3(mposition * glm::vec4(lpos, 1.0f));
			lightControl->lightModelPos = glm::translate(glm::mat4(1.0f), lpos);//更新光源位置
...
#undef max
		...
		*vertices = new float[*vsize];
		...
		float r1 = colorMaps[0][0];
		float r2 = colorMaps[0][1];
		float r3 = colorMaps[0][2];
		float r4 = colorMaps[1][0];
		float r5 = colorMaps[1][1];
		float r6 = colorMaps[1][2];
		//获取到模型的中心坐标位置
