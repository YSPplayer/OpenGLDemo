...
namespace GL {
	...
	public:
		...
		inline void SetModelCenterPoisition(const glm::vec3& centerPosition) {
			...
			this->changeCenterPosition = centerPosition;
		};
		inline glm::vec3 GetModelCenterPoisition() {
			return centerPosition;
		}
		glm::mat4 UpdatePoisition(Data& data, Camera* cmaera);
	...
	private:
		...
		float* tangents;//法线切线数组
		float* bitangents;//法线副切线数组
		unsigned int* indices;//存放顶点数据的索引位数据
		int uvsSize;//UV贴图数据的大小
		...
		glm::vec3 changeCenterPosition;//模型的中心位置
	};
}