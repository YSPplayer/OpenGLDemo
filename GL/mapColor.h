	...
	public:
		...
		static void SetColorForZ(float**&colorMaps,float fraction,int index, std::vector<MapColor>& mapColor) {
			...
			for (int i = 0; i < mapColor.size(); i++) {
				float r = static_cast<float>(mapColor[i].ColorR(ugrey)) / 255.0f;
				float g = static_cast<float>(mapColor[i].ColorG(ugrey)) / 255.0f;
				float b = static_cast<float>(mapColor[i].ColorB(ugrey)) / 255.0f;
